...
public class DBConnectionTest {
    public static void main(String[] args) throws Exception {
        ...
        Statement stmt  = conn.createStatement(); // 연결된 DB에 SQL 명령을 내리기 위해 Statement를 생성한다.

        String query = "SELECT now()"; // 시스템의 현재 날짜시간을 출력하는 쿼리(query)
        ResultSet rs = stmt.executeQuery(query); // query를 실행한 결과를 rs에 담는다. ResultSet는 2차원 테이블 형태!

        // 실행결과가 담긴 rs에서 한 줄씩 읽어서 출력
        while (rs.next()) {
            String curDate = rs.getString(1);  // 읽어온 행의 첫번째 컬럼의 값을 String으로 읽어서 curDate에 저장
            System.out.println(curDate);       // 2024-01-06 16:01:07
        }
    } // main()
}
