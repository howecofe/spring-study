...
public class HomeController {
	@Autowired
	WebApplicationContext servletAC; // 1. Servlet AC는 @Autowired 로 주입 받는다.
	...
	public String home(Locale locale, HttpServletRequest request, Model model) {
		...
		WebApplicationContext rootAC = WebApplicationContextUtils.getWebApplicationContext(sc); // 2. Root AC는 메서드를 이용해서 얻는다.
		//  Root AC는 servletAC.getParent()를 이용해서 구할 수도 있다.
...
/* [실행결과]
webApplicationContext = Root WebApplicationContext: startup date [Fri Jan 05 12:31:42 KST 2024]; root of context hierarchy
servletAC = WebApplicationContext for namespace 'appServlet-servlet': startup date [Fri Jan 05 12:31:43 KST 2024]; parent: Root WebApplicationContext
